...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
             ...
             if (reminder.type === 'interval') {
                 ...
                 if (reminder.targetDateTime && reminder.targetDateTime > now + 1000) {
                     ...
                     setCustomTimerStates(prev => ({
                         ...
                         [reminder.id]: {
                             timeLeft: tLeft,
                             endTime: reminder.targetDateTime ?? null
                         }
                     }));
                 }
             }
  ...
  useEffect(() => {
    ...
    setCustomTimerStates(prev => {
      ...
      settings.customReminders.forEach(r => {
        if (activeAlerts.has(r.id)) return;
  ...
  }, [settings.customReminders, activeAlerts]);
    ...
    timerRef.current = window.setInterval(() => {
        ...
        setCustomTimerStates(prev => {
            ...
            settings.customReminders.forEach(r => {
                ...
                if (diff <= 0) {
                    next[r.id].timeLeft = 0;
                    
                    if (activeAlerts.has(r.id)) {
                        // Paused waiting for dismiss
                    } else {
                        triggerAlert(r.id);
                        if (r.type === 'onetime') {
                            next[r.id].endTime = null; 
                        }
                    }
                    stateChanged = true;
                } else {
  ...
  // Add listener for notification close from IPC (Main Process)
  useEffect(() => {
